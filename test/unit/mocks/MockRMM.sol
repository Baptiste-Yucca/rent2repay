...
contract MockRMM is IRMM {
        ...
        returns (uint256)
    {
        ...
        require(
            IERC20(debtToken).transferFrom(
                onBehalfOf, address(0x000000000000000000000000000000000000dEaD), actualRepaidAmount
            ),
        ...
        require(
            IERC20(asset).transferFrom(msg.sender, address(0x000000000000000000000000000000000000dEaD), amount),
            "Transfer from failed"
        );
    ...
    function withdraw(address asset, uint256 amount, address to) external override returns (uint256) {
        ...
        require(
            IERC20(supplyToken).transferFrom(msg.sender, address(0x000000000000000000000000000000000000dEaD), amount),
            "Transfer from failed"
        );
    ...
    function supply(address asset, uint256 amount, address onBehalfOf, uint16 /* referralCode */ ) external override {
        ...
        require(IERC20(asset).transferFrom(msg.sender, address(this), amount), "Transfer asset failed");
