...
contract giveApprovalScript is Script {
    function run() external {
        ...
        rent2Repay.giveApproval(usdcAddr, rmmAddress, 0); //type(uint256).max);
        rent2Repay.giveApproval(wxdaiAddr, rmmAddress, 0); //type(uint256).max);
        rent2Repay.giveApproval(wxdaiSupplyAddr, rmmAddress, 0); //type(uint256).max);
        rent2Repay.giveApproval(usdcSupplyAddr, rmmAddress, 0); //type(uint256).max);
